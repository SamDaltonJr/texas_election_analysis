"""
Parse Texas Election Results by State Senate District from PDFs
...
def parse_senate_pdf(pdf_path, year):
    """Parse State Senate district election PDF for any year"""
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_num in race_pages:
            ...
            for row in table:
                ...
                # Only process STATE and numeric districts (1-31 for State Senate)
...
def main():
    print("="*70)
    print("Texas State Senate District Election Data Parser")
    print("="*70)
    ...
    files_to_parse = [
        {
            'year': 2018,
            'path': f"{base_path}/2018_senate_s2168.pdf",
            'output': f"{base_path}/2018_senate_results.csv"
        },
        {
            'year': 2020,
            'path': f"{base_path}/2020_senate_s2168.pdf",
            'output': f"{base_path}/2020_senate_results.csv"
        },
        {
            'year': 2022,
            'path': f"{base_path}/2022_senate_s2168.pdf",
            'output': f"{base_path}/2022_senate_results.csv"
        },
        {
            'year': 2024,
            'path': f"{base_path}/2024_senate_s2168.pdf",
            'output': f"{base_path}/2024_senate_results.csv"
        }
    ]
    ...
    for file_info in files_to_parse:
        ...
        results = parse_senate_pdf(file_info['path'], file_info['year'])
    ...
    if all_results:
        ...
        combined_output = f"{base_path}/2018_2024_senate_results_combined.csv"
        ...
        # Show sample
        print(f"\n{'='*70}")
        print("Sample Data - 2024 Presidential Results, District 10 (Austin)")
        print("="*70)
        sample = combined_df[(combined_df['year'] == 2024) &
                            (combined_df['district'] == '10') &
